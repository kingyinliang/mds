...
<template>
    <div class="header_main">
        <mds-card title="温度控制" :pack-up="false">
            <template slot="titleBtn">
                <div style="float: right; height: 32px; margin-bottom: 10px;">
                    <el-select v-model="queryForm.materialCode" placeholder="请选择生产物料" size="small" filterable tyle="width: 160px;" suffix-icon="el-icon-search" clearable>
        ...
        <el-dialog width="500px" title="高级查询" :close-on-click-modal="false" :visible.sync="visibleHightLevelQuery">
            <el-form :inline="true" size="small" :model="queryForm" label-width="100px">
                <el-form-item label="生产物料：">
                    <el-select v-model="queryForm.materialCode2" placeholder="请选择" filterable style="width: 283px;" clearable>
                        <el-option v-for="(sole, index) in productMaterialList" :key="index" :value="sole.materialCode" :label="`${sole.materialName} ${sole.materialCode}`" />
                    </el-select>
                </el-form-item>
                <el-form-item label="温度类型：">
                    <el-select v-model="queryForm.type" placeholder="请选择" filterable style="width: 283px;" clearable @change="v => getControlTypeChange(v)">
                        <el-option v-for="(sole, index) in controlTypeList" :key="index" :value="sole.dictCode" :label="sole.dictValue" />
                    </el-select>
                </el-form-item>
                <el-form-item label="温度阶段：">
                    <el-select v-model="queryForm.stage" placeholder="请选择" filterable style="width: 283px;" clearable>
        ...
        <add-or-update ref="addOrUpdate" :work-shop-list="workShopList" :product-material-list="productMaterialList" :control-type-list="controlTypeList" @refreshDataList="refreshDataList" />
    </div>
</template>
...
<script lang="ts">
    ...
    export default class TemperatureStep extends Vue {
        ...
        // 获取温度类型，数据字典内HEAT、cool和DISCHARGE的参数类型
        getControlTypeList() {
        ...
        highSearch() {
            this.queryForm.materialCode2 = this.queryForm.materialCode;
        ...
        getData() {
            ...
            if (this.queryType === 1) {
                ...
                params = {
                    ...this.queryForm,
                    materialCode: this.queryForm.materialCode2
                }
            }
            STE_API.STE_CRAFT_QUERY_API(params).then(({ data }) => {
        ...
        addOrUpdateHandler(type, row) {
            ...
            if (type === 'add') {
                ...
                this.$refs.addOrUpdate.getControlTypeChange(row.type)
            }
        ...
        remove() {
            if (this.multipleSelection.length === 0) {
                ...
                this.$confirm('确认删除, 是否继续?', '删除', {
                    ...
                    STE_API.STE_CRAFT_DEL_API(this.multipleSelection).then(() => {
