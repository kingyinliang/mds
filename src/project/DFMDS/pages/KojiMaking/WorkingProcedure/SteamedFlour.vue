    ...
    <div>
        ...
        >
            <template slot="1" slot-scope="data">
                <flour-material-apply ref="flourMaterialApply" :is-redact="data.isRedact" />
            </template>
            <template slot="2" slot-scope="data">
                <flour-material-craft ref="flourMaterialCraft" :is-redact="data.isRedact" />
            </template>
...
<script lang="ts">
    ...
    export default class SteamedFlourIndex extends Vue {
        ...
        submitRules(): Function[] {
            return [this.$refs.flourMaterialApply.ruleSubmit, this.$refs.excRecord.ruleSubmit]
        }
        ...
        getOrderList() {
            STE_API.STE_DETAIL_CRAFTHEADER_INFO_API({
                ...
                this.$refs.excRecord.init(this.formHeader, 'flourMaterialApply');
                this.$refs.textRecord.init(this.formHeader, 'sterilize');
            })
        }

        savedDatas() {
            const steSemi = this.$refs.flourMaterialApply.savedData(this.formHeader);
            const excRequest = this.$refs.excRecord.getSavedOrSubmitData(this.formHeader, 'flourMaterial');
