            ...
            <el-col v-for="item in stockInfoList" :key="item.potId" :span="12">
                <div class="card-stock">
                    ...
                    <div class="card-stock__content">
                        <stock-detail-num ref="stockDetailNum" :current-data="item" />
...
<script lang="ts">
    ...
    @Component({
        name: 'StockFlourIndex',
        components: {
            StockDetailTable,
            StockDetailNum
        }
    })
    export default class StockFlourIndex extends Vue {
        // 查询参数集
        ...
        private getDataList() {
            ...
            if (this.queryParams.workShopId) {
                KOJI_API.KOJI_STOCK_FLOUR_INDEX_LIST_API({
                    ...this.queryParams
                }).then(({ data }) => {
                    this.FnCallBackHandle(data);
                });
            } else {
                KOJI_API.KOJI_STOCK_FLOUR_INDEX_LIST_ALL_API({
                    //Wildcard: this.queryParams.workShopId
                }).then(({ data }) => {
                    this.FnCallBackHandle(data);
                });
            }
        ...
        private goDetail(row) {
            this.$store.commit('koji/updateStockFlourInfo', row);
            this.$store.commit(
                'common/updateMainTabs',
                this.$store.state.common.mainTabs.filter(subItem => subItem.name !== 'DFMDS-pages-KojiMaking-Stock-Flour-detail')
            );
            setTimeout(() => {
                this.$router.push({
                    name: `DFMDS-pages-KojiMaking-Stock-Flour-detail`
                });
            }, 100);
        }
    }
