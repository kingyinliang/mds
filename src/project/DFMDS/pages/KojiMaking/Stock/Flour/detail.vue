    ...
    <div class="header_main">
        ...
        <el-tabs id="DaatTtabs" ref="tabs" v-model="activetTabName" class="NewDaatTtabs tabsPages" type="border-card">
            <el-tab-pane name="1">
                ...
                <stock-table-data :work-shop-info="$store.state.koji.StockFlourInfo" query-auth="flourMxQuery" adjust-auth="flourAdjust" stock-type="FLOUR" />
            </el-tab-pane>
            <el-tab-pane name="2">
                <span slot="label" class="spanview">
                    历史库存信息
                </span>
                <stock-table-data :work-shop-info="$store.state.koji.StockFlourInfo" query-auth="flourMxQuery" adjust-auth="flourAdjust" :is-history-page="true" stock-type="FLOUR" />
            </el-tab-pane>
        </el-tabs>
    </div>
</template>
...
<script lang="ts">
    ...
    @Component({
        ...
        name: 'StockFlourDetail'
    })
    export default class StockFlourDetail extends Vue {
        // 默认第一个标签
        ...
        mounted() {
            this.stockInfoObj = {
                ...this.$store.state.koji.StockFlourInfo
            };
