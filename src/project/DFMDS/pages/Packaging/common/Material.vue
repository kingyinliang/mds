        ...
        <mds-card :title="'包材领用'" :name="'materialP'">
            <el-table header-row-class-name="tableHead" class="newTable" :data="currentDataTable" :row-class-name="rowDelFlag" :span-method="spanMethod" border tooltip-effect="dark">
                ...
                <el-table-column label="批次" prop="batch" width="150">
                    ...
                    <template slot-scope="scope">
                        <div v-if="scope.row.materialType =='ZVER'">
                        ...
                        <div v-else>
                            <el-input v-model="scope.row.batch" :disabled="!(isRedact && scope.row.checkStatus !== 'C' && scope.row.checkStatus !== 'D' && scope.row.checkStatus !== 'P' && scope.row.materialStatus !== '3')" size="small" placeholder="请输入" />
                        </div>
                    </template>
                </el-table-column>
