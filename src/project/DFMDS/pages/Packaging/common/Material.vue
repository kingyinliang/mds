        ...
        <mds-card v-for="(item, index) in materialSArr" :key="index" :title="'灌装线' + (index + 1)" :name="'materialS' + index">
            <el-table ref="materialS" header-row-class-name="tableHead" class="newTable" max-height="267" :data="item.data" :row-class-name="rowDelFlag" border tooltip-effect="dark">
                ...
                <el-table-column label="开始使用时间" width="195">
                    ...
                    <template slot-scope="scope">
                        <el-date-picker v-model="scope.row.startDate" :disabled="!(isRedact && status !== 'C' && status !== 'D' && status !== 'P' && scope.row.materialStatus !== '3')" type="datetime" value-format="yyyy-MM-dd" format="yyyy.MM.dd" size="small" style="width: 170px;" />
                    </template>
                </el-table-column>
                <el-table-column label="用完时间" prop="batch" width="195">
                    <template slot-scope="scope">
                        <el-date-picker v-model="scope.row.endDate" :disabled="!(isRedact && status !== 'C' && status !== 'D' && status !== 'P' && scope.row.materialStatus !== '3')" type="datetime" value-format="yyyy-MM-dd" format="yyyy.MM.dd" size="small" style="width: 170px;" />
                    </template>
                </el-table-column>
                <el-table-column label="备注" prop="remark" min-width="140">
                    <template slot-scope="scope">
