            ...
            <template slot="tab-head0">
                <div class="tab__heads clearfix">
                    ...
                    <div style="float: right;">
                        <span>过账日期：</span>
                        <el-date-picker v-model="pstngDate" value-format="yyyy-MM-dd" format="yyyy-MM-dd" size="small" style="width: 140px; margin-right: 10px;" />
                        <span>抬头文本：</span>
                        <el-input v-model="headerText" placeholder="抬头文本" size="small" style="width: 190px; margin-right: 10px;" />
    ...
    getPass() {
        ...
        if (this.$refs.queryTable.tabs[0].multipleSelection && this.$refs.queryTable.tabs[0].multipleSelection.length) {
            ...
            this.$confirm('确认过账，是否继续', '过账确认', {
                ...
                AUDIT_API.PROISSUEPASS_API(list)
                    .then(({ data }) => {
    ...
    getRefuseOrWriteOffs() {
        if (this.dialogTitle === '退回原因') {
            ...
            this.$confirm('数据已经调用SAP接口已发料，反审后将冲销SAP数据，是否反审?', '反审确认', {
                ...
                AUDIT_API.PROISSUEWRITEOFFS_API(list)
                    .then(({ data }) => {
                        this.isRefuseOrWriteOffsDialogShow = false;
                        this.$successToast(data.msg);
                        this.$refs.queryTable.getDataList(true);
