...
Log.writeErrorLog = function (ex, extra, tags, user) {
  ...
  Sentry.configureScope(scope => {
    const id = sessionStorage.getItem('vuex') ? JSON.parse(sessionStorage.getItem('vuex')).user.id : ''
    const name = sessionStorage.getItem('vuex') ? `${JSON.parse(sessionStorage.getItem('vuex')).user.realName}（${JSON.parse(sessionStorage.getItem('vuex')).user.name}）` : ''
    scope.setUser({
      id: id,
      name: name,
      email: ''
