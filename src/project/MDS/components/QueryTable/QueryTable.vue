<template>
    <div>
        <el-card class="searchCard" style="margin-bottom: 5px;">
            <el-form :model="queryForm" :rules="queryFormRules" :inline="true" size="small" labelWidth="70px" class="multi_row clearfix" style="font-size: 0;">
                <template v-for="item in queryFormData">
                    <template v-if="!item.hide">
                        ...
                        <el-form-item v-if="item.type === 'date-interval'" :key="item.prop" class="dateinput" :label="`${item.label}：` || ''" :prop="item.prop">
                            <el-row>
                                <el-col :span="12">
                                    <el-date-picker :ref="item.prop" v-model="queryForm[item.prop]" :type="item.dataType ? item.dataType : 'date'" placeholder="选择日期" :valueFormat="item.valueFormat ? item.valueFormat : 'yyyy-MM-dd'" style="width: 135px;" />
                                    <span>-</span>
                                </el-col>
                                <el-col :span="12">
                                    <el-date-picker :ref="item.propTwo" v-model="queryForm[item.propTwo]" :type="item.dataType ? item.dataType : 'date'" placeholder="选择日期" :valueFormat="item.valueFormat ? item.valueFormat : 'yyyy-MM-dd'" style="width: 135px;" />
                                </el-col>
                            </el-row>
                        </el-form-item>
                        <el-form-item v-if="item.type === 'date-picker'" :key="item.prop" :label="`${item.label}：` || ''" :prop="item.prop">
                            <el-date-picker :ref="item.prop" v-model="queryForm[item.prop]" :type="item.dataType" placeholder="请选择" :valueFormat="item.valueFormat" style="width: 170px;" />
                        </el-form-item>
                    </template>
                </template>
        ...
        <el-tabs v-if="tabs.length" v-model="activeName" type="border-card">
            <el-tab-pane v-for="(tabItem, index) in tabs" :key="index" :name="index.toString()" :label="tabItem.label">
                <el-table ref="table" :data="tabItem.tableData" height="400" border tooltipEffect="dark" headerRowClassName="tableHead" style="width: 100%; margin-bottom: 20px;" @selection-change="handleSelectionChange">
        ...
        <el-card v-if="!tabs.length" class="tableCard" style="min-height: 400px;">
            ...
            <el-table ref="table" :data="tableData" :height="tableHeight" :spanMethod="spanMethod" border tooltipEffect="dark" headerRowClassName="tableHead" style="width: 100%; margin-bottom: 20px;" @selection-change="handleSelectionChange">
                <el-table-column v-if="showSelectColumn" :selectable="selectableFn" type="selection" width="50px" />
                <el-table-column v-if="showIndexColumn" type="index" :index="indexMethod" label="序号" width="50px" />
            ...
            <el-row v-if="showPage === true">
                <el-pagination :currentPage="queryForm.currPage" :pageSizes="[10, 20, 50]" :pageSize="queryForm.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="queryForm.totalCount" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
            </el-row>
        </el-card>
    </div>
</template>
