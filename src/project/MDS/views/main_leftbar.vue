    ...
    <aside class="site-sidebar" :class="'site-sidebar--' + sidebarLayoutSkin" @mouseenter="showMenu" @mouseleave="closeMenu">
        <div class="site-sidebar__inner">
            <el-menu :default-active="menuActiveName || 'home'" :collapse="sidebarFold" :collapse-transition="false" class="site-sidebar__menu">
                <el-menu-item
                    index="home"
                    @click="menuActiveName = 'home'; $router.history.current.name==='home'?'':$router.push({ path: 'home' });"
                >
                    <i class="site-sidebar__menu-icon iconfont factory-shouye" />
                    <span slot="title">首页</span>
                </el-menu-item>
                <template v-for="menu in menuList">
                    <sub-menu v-if="menu.type != '4'" :key="menu.menuId" :menu="menu" :dynamic-menu-routes="dynamicMenuRoutes" />
                </template>
            </el-menu>
        </div>
    </aside>
