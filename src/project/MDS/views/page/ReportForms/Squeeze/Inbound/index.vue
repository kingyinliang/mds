                        ...
                        <el-col>
                            <el-form :model="params" size="small" :inline="true" labelPosition="right" labelWidth="70px" class="multi_row">
                                ...
                                <el-form-item label="原汁罐：">
                                    <el-select v-model="params.potNo" class="selectwpx" style="width: 140px;" filterable @change="changeOptions('pot')">
                                        <el-option label="请选择" value="" />
                                ...
                                <el-form-item>
                                    <template>
                                        <el-radio v-model="params.mixType" label="正常" style="margin-right: 10px;">
                                            正常罐
                                        </el-radio>
                                        <el-radio v-model="params.mixType" label="混合">
                                            混合罐
                                        </el-radio>
                                    </template>
                                </el-form-item>
                ...
                <el-row v-show="searched" style="margin-top: 5px;">
                    <div style="min-height: 320px;">
                        <el-table border headerRowClassName="tableHead" :data="dataList">
                            ...
                            <el-table-column label="原汁罐" width="100" :showOverflowTooltip="true">
                                <template slot-scope="scope">
                                    {{ scope.row.potName }}
                                </template>
                            </el-table-column>
                            ...
                            <el-table-column label="发酵罐号" width="100" :showOverflowTooltip="true">
                                <template slot-scope="scope">
...
export default class Index extends Vue {
    ...
    changeOptions(flag: string) {
        if (flag === 'factory') {
            ...
            const item = this.potList.find(ele => ele['holderId'] === this.params.potNo);
    ...
    exportExcel() {
        ...
        exportFile(`${REP_API.INSTOCK_MATERIAL_EXPECT_API}`, '压榨入库报表', this);
    }
    ...
    getDataList(params) {
        this.dataList = [];
        Vue.prototype
            .$http(`${REP_API.INSTOCK_MATERIAL_LIST_API}`, 'POST', params)
            .then(res => {
                if (res.data.code === 0) {
