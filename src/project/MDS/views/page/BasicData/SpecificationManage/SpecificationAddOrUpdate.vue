<template>
    <el-dialog :title="SpecificationId ? '修改规格信息' : '新增规格'" :close-on-click-modal="false" :visible.sync="visible">
        <div>
            <el-form ref="dataForm" :model="dataForm" status-icon :rules="dataRule" label-width="100px" size="small" @keyup.enter.native="dataFormSubmit()">
                ...
                <el-form-item label="物料：" prop="material">
                    <el-select v-if="!SpecificationId" v-model="dataForm.material" filterable placeholder="请选择" style="width: 100%;" @change="setBrand">
                        <el-option v-for="item in serchSapList" :key="item.itemId" :label="item.sapCode + ' ' + item.itemName + ' ' + item.kondm" :value="item.sapCode + ' ' + item.itemName + ' ' + item.kondm" />
                    </el-select>
