                ...
                <el-col :span="24">
                    <el-form :model="form" :inline="true" size="small" labelWidth="70px" class="multi_row">
                        ...
                        <el-form-item label="申请编码：">
                            <el-input v-model="form.applyNo" style="width: 199px;" />
                        </el-form-item>
        ...
        <el-tabs v-model="activeName" type="border-card" style="margin-top: 5px;" @tab-click="tabClick">
            <el-tab-pane name="0" label="未确认">
                <el-table :data="dataList" border headerRowClassName="tableHead">
                    ...
                    <el-table-column label="申请数量" prop="amount" />
                    <el-table-column label="申请时间" prop="created" width="170" />
                    <el-table-column label="生产日期" prop="productDate" />
                    <el-table-column label="操作" width="100">
                        <template slot-scope="scope">
                            <el-button type="text" size="small" :disabled="!isAuth('fer:openholderg:confirm')" @click="Go(scope.row)">
                                确认
                            </el-button>
                            <el-button type="text" size="small" :disabled="!isAuth('fer:openholderg:deleteG')" @click="DelRow(scope.row)">
                                删除
                            </el-button>
                        </template>
                    </el-table-column>
            ...
            <el-tab-pane name="1" label="已确认">
                <el-table :data="dataList" border headerRowClassName="tableHead">
                    ...
                    <el-table-column label="半成品类别" prop="halfType" />
                    <el-table-column label="申请数量" prop="amount" />
                    <el-table-column label="确认数量" prop="confirmNum" />
                    <el-table-column label="申请时间" prop="created" width="170" />
                    <el-table-column label="生产日期" prop="productDate" />
                </el-table>
                <el-pagination :currentPage="form.currPage" :pageSizes="[10, 20, 50]" :pageSize="form.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="form.totalCount" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
...
export default {
    ...
    methods: {
        ...
        //  生产物料
        GetMaterialType(n) {
            this.$http(`${FERMENTATION_API.FORRECIPIENTSHOLDER_API}`, 'POST', {
                factory: n
            }).then(({ data }) => {
                if (data.code === 0) {
