<template>
    <div class="header_main">
        <el-card class="searchCard ferCard">
            <el-form :inline="true" :model="formHeader" size="small" labelWidth="70px" class="multi_row">
        ...
        <el-card v-show="fastS" class="searchCard newCard ferCard" style="margin-top: 5px;">
            <h3 class="cardTit">
                <i class="iconfont factory-shujuzonglan" style="color: #666; margin-right: 10px;" />原汁情况总览
                        ...
                        <div class="topBox_boxItem" @click="topClick(item)">
                            <div class="topBox_boxItem_bar">
                                <div
                                    ...
                                    :style="{ background: `linear-gradient(to right,${item.startColor} 0%,${item.startColor} 10%,${item.endColor})` }"
                                />
                            </div>
                            ...
                            <div v-if="item.content !== 0" class="topBox_boxItem_popover">
                                <p v-for="(i, ins) in Object.keys(item.content)" :key="ins">
                                    <i
                                        ...
                                        class="dot"
                                        :style="{
                                            background: ins === 0 ? '#1890FF' : ins === 1 ? '#FFBF00' : '#1890FF',
                                        }"
                                    />{{ i }}
                                    <span v-if="i != ''" style="float: right;">{{ item.content[i] }}方</span>
                                </p>
        ...
        <el-card v-show="fastS" class="searchCard newCard ferCard" style="margin-top: 5px;">
            <h3 style="color: black; margin-bottom: 8px;">
                <i class="iconfont factory-liebiao" style="color: #666; margin-right: 10px;" />原汁罐列表
                ...
                <el-col v-for="(item, index) in dataList" :key="index" :span="4">
                    <el-card class="dataList_item" style="padding: 0 !important;">
                        ...
                        <div class="dataList_item_pot clearfix" style="position: relative;">
                            <img v-if="item.IS_F === '1'" src="@/assets/img/F0.png" alt="" style="position: absolute; left: 10px; top: 10px;">
                            <img v-if="item.isRd === 1" src="@/assets/img/RD.png" alt="" style="position: absolute; left: 10px; top: 10px;">
                            <img v-if="item.IS_F === '2'" src="@/assets/img/jbs.png" alt="" style="position: absolute; left: 10px; top: 10px;">
            ...
            <el-row>
                <el-pagination :currentPage.sync="pages.currentPage" :pageSize="42" layout="total, prev, pager, next" :total="pages.total" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
            </el-row>
        </el-card>
        <el-dialog :visible.sync="TransferDialogTableVisible" :closeOnClickModal="false" width="500px" customClass="dialog__class">
            <div slot="title">
                转储
            </div>
            <div>
                <el-form ref="Transferstar" size="small" :model="formTransfer" :rules="Transferulestar" labelWidth="150px">
                    ...
                    <el-form-item label="满灌时间：">
                        <el-date-picker v-model="formTransfer.fullDate" type="datetime" placeholder="请选择" style="width: 200px;" format="yyyy-MM-dd HH:mm" valueFormat="yyyy-MM-dd HH:mm" />
                    </el-form-item>
                </el-form>
            </div>
        ...
        <el-dialog :visible.sync="AddDialogTableVisible" :closeOnClickModal="false" width="500px" customClass="dialog__class">
            <div slot="title">
                HD
            </div>
            <div>
                <el-form ref="Addstar" size="small" :model="formAdd" :rules="Addrulestar" labelWidth="150px">
                    <el-form-item label="领用罐号：">
                        {{ formAdd.holderName }}
                    </el-form-item>
                    <el-form-item label="物料：">
        ...
        <el-dialog :closeOnClickModal="false" :visible.sync="JudgeDialogTableVisible" width="400px" customClass="dialog__class">
            <div slot="title">
                类别判定
            </div>
        ...
        <el-dialog width="450px" class="ShinHoDialog" :title="dialogData.HOLDER_NAME + '清洗'" :closeOnClickModal="false" :visible.sync="visible">
            <div style="display: flex;">
                <el-form :model="dialogData" labelWidth="100px" class="topform marbottom" style="margin: auto;">
        ...
        <el-dialog :visible.sync="BringOutDialogTableVisible" :closeOnClickModal="false" width="500px" customClass="dialog__class">
            <div slot="title">
                调整
            </div>
            <div>
                <el-form ref="BringOutstar" size="small" :model="formBringOut" :rules="BringOutrulestar" labelWidth="150px">
                    <el-form-item label="领用罐号：">
                    ...
                    <el-form-item v-if="formBringOutLable" :label="formBringOutLable + '：'">
                        <el-select v-model="formBringOut.factoryHolder" placeholder="请选择" filterable clearable :disabled="!formBringOutPot.length" style="width: 200px;">
                            <template v-for="(sole, index) in formBringOutPot">
                                <el-option v-if="formBringOut.adjustType === '调拨'" :key="index" :value="sole.code" :label="sole.value" />
                            </template>
                            <template v-for="(sole, index) in formBringOutPot">
                                <el-option v-if="formBringOut.adjustType === '调配转入'" :key="index" :value="sole.holderId" :label="sole.holderName" />
                            </template>
                            <template v-for="(sole, index) in formBringOutPot">
                                <el-option v-if="formBringOut.adjustType === 'HD'" :key="index" :value="sole.holderId" :label="sole.HOLDER_NAME" />
                            </template>
                        </el-select>
                    </el-form-item>
                    ...
                    <el-form-item label="操作时间：">
                        <el-date-picker v-model="formBringOut.bringTime" type="datetime" valueFormat="yyyy-MM-dd HH:mm" format="yyyy-MM-dd HH:mm" placeholder="请选择" style="width: 199px;" />
                    </el-form-item>
                </el-form>
            </div>
            <span slot="footer" class="dialog-footer">
