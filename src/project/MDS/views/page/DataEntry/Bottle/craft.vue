    ...
    <div class="header_main">
        ...
        <el-tabs ref="tabs" v-model="activeName" type="border-card" class="NewDaatTtabs secondcard" @tab-click="tabClick">
            <el-tab-pane name="1" label="产品参数">
                ...
                <el-table :data="productList" :rowClassName="rowDelFlag" border headerRowClassName="tableHead" style="margin-top: 10px;">
                    <el-table-column type="index" label="序号" width="50" fixed="left" />
                    <el-table-column label="产品参数" fixed="left" showOverflowTooltip width="85" prop="parameter" />
                    <el-table-column label="时间" width="200">
                        <template slot-scope="scope">
                            <el-date-picker v-model="scope.row.time" type="datetime" valueFormat="yyyy-MM-dd HH:mm" format="yyyy-MM-dd HH:mm" style="width: 180px;" placeholder="请选择日期" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="1#" width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.oneWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="2#" width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.twoWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="3#" width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.threeWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="4#" width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.fourWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="5#" width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.fiveWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="6#" width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.sixWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="7#" width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.sevenWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="8#" showOverflowTooltip width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.eightWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="9#" showOverflowTooltip width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.nineWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="10#" showOverflowTooltip width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.tenWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="11#" showOverflowTooltip width="100">
                        <template slot-scope="scope">
                            <el-input v-model="scope.row.elevenWell" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
            ...
            <el-tab-pane name="2" label="设备数据">
                ...
                <el-table :data="equipmentList" :rowClassName="rowDelFlag" border headerRowClassName="tableHead" style="margin-top: 10px;">
                    <el-table-column type="index" label="序号" width="50" />
                    <el-table-column label="时间">
                        <template slot-scope="scope">
                            <el-date-picker v-model="scope.row.date" type="datetime" :disabled="!isRedact" valueFormat="yyyy-MM-dd HH:mm" format="yyyy-MM-dd HH:mm" style="width: 180px;" placeholder="请选择日期" size="small" />
                        </template>
                    </el-table-column>
                    <el-table-column label="电压V">
                        <template slot-scope="scope">
                            <el-input v-model.number="scope.row.voltage" type="text" onkeyup="value=value.replace(/^(0+)|[^\d]+/g,'')" :disabled="!isRedact" size="small" />
                        </template>
                    </el-table-column>
            ...
            <el-tab-pane name="3" label="加温参数">
                ...
                <el-table :data="warmingList" :rowClassName="rowDelFlag" border headerRowClassName="tableHead" style="margin-top: 10px;">
                    <el-table-column type="index" label="序号" width="50" />
                    <el-table-column label="时间" width="200">
                        <template slot-scope="scope">
                            <el-date-picker v-model="scope.row.date" type="datetime" :disabled="!isRedact" valueFormat="yyyy-MM-dd HH:mm" format="yyyy-MM-dd HH:mm" style="width: 180px;" placeholder="请选择日期" size="small" />
...
export default {
    ...
    methods: {
        ...
        GetProductList() {
            this.$http(`${BOTTLE_API.BOTTLE_CRAFT_PRODUCT_LIST}`, 'POST', {
                orderId: this.orderId,
                pageSize: this.pages.pageSize,
                currPage: this.pages.currPage,
                totalCount: this.pages.totalCount
            }).then(({ data }) => {
                if (data.code === 0) {
        ...
        GetEquipmentList() {
            this.$http(`${BOTTLE_API.BOTTLE_CRAFT_EQUIPMENT_LIST}`, 'POST', {
                orderId: this.orderId
            }).then(({ data }) => {
                if (data.code === 0) {
        ...
        GetWarmingList() {
            this.$http(`${BOTTLE_API.BOTTLE_CRAFT_WARMING_LIST}`, 'POST', {
                orderId: this.orderId
            }).then(({ data }) => {
                if (data.code === 0) {
        ...
        AddProductRow() {
            const productList = this.productList.filter(item => {
            ...
            if (productList.length === 0) {
                ...
                const NewList = productList.slice(0, 6);
        ...
        AddEquipmentRow() {
            const NewequipmentList = this.equipmentList.filter(item => {
                return item.delFlag === '0';
            });
        ...
        AddWarmingRow() {
            const NewarmingList = this.warmingList.filter(item => {
                return item.delFlag === '0';
            });
        ...
        SaveHeader(str, resolve, reject) {
            this.$http(`${BOTTLE_API.BOTTLE_PRO_HEAD_UPDATE}`, 'POST', {
                orderId: this.orderId,
                type: 'craft'
            }).then(({ data }) => {
                if (data.code === 0) {
