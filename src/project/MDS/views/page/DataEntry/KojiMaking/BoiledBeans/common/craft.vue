<template>
    <div>
        <mds-card :title="'生产时间'" :name="'craftTime'">
            <el-form :inline="true" :model="craftfrom" size="small" label-width="130px">
                <el-form-item label="预热开始时间：" :required="true">
                    <el-date-picker v-model="craftfrom.preheatDate" type="datetime" :disabled="!(isRedact && craftfrom.status !== 'submit' && craftfrom.status !== 'checked')" placeholder="选择日期" size="small" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" style="width: 171px;" />
                </el-form-item>
                <el-form-item label="下料开始时间：" :required="true">
                    <el-date-picker v-model="craftfrom.unloadingStartDate" type="datetime" :disabled="!(isRedact && craftfrom.status !== 'submit' && craftfrom.status !== 'checked')" placeholder="选择日期" size="small" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" style="width: 171px;" />
                </el-form-item>
                <el-form-item label="下料结束时间：" :required="true">
                    <el-date-picker v-model="craftfrom.unloadingEndDate" type="datetime" :disabled="!(isRedact && craftfrom.status !== 'submit' && craftfrom.status !== 'checked')" placeholder="选择日期" size="small" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" style="width: 171px;" />
                </el-form-item>
        ...
        <mds-card :title="'煮豆润水参数设置'" :name="'rateRunWater'" :icon-bg="'#ffbf00'">
        ...
        <mds-card :title="'连续蒸煮记录'" :name="'lianxu'">
            ...
            <el-table ref="zhengzhuTable" class="newTable borderTable" max-height="267" border header-row-class-name="tableHead" :data="zhengzhuList" :row-class-name="rowDelFlag">
                <el-table-column min-width="175">
                    ...
                    <template slot-scope="scope">
                        <el-date-picker v-model="scope.row.guardTime" type="datetime" :disabled="!(isRedact && craftfrom.status !== 'submit' && craftfrom.status !== 'checked')" placeholder="选择时间" size="small" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm" style="width: 171px;" />
                    </template>
                </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getList(formHeader) {
                this.$http(`${KJM_API.DOUGONGYILISTSAVE_API}`, 'POST', { orderHouseId: formHeader.orderHouseId })
                    .then(({ data }) => {
                        if (data.code === 0) {
