<template>
    <div>
        <mds-card :title="'种曲'" :name="'material'">
            ...
            <el-table ref="materialTable" class="newTable" border maxHeight="267" style="margin-top: 10px;" headerRowClassName="tableHead" :data="materialList" :rowClassName="rowDelFlag">
                <el-input type="index" />
        ...
        <mds-card :title="'小麦粉'" :name="'wheat'" :iconBg="'#ffbf00'">
                        ...
                        <el-popover placement="top" title="标题" width="200" trigger="hover">
                            <div class="boxText">
                                <div v-for="(soles, index) in sole.pici" :key="index">
                                    ...
                                    <el-progress v-if="index % 3 === 0" :percentage="soles.proportion" :showText="false" :textInside="true" :strokeWidth="8" color="#1890FF" />
                                    <el-progress v-else-if="(index - 1) % 3 === 0" :percentage="soles.proportion" :showText="false" :textInside="true" :strokeWidth="8" color="#5BD171" />
                                    <el-progress v-else-if="(index - 2) % 3 === 0" :percentage="soles.proportion" :showText="false" :textInside="true" :strokeWidth="8" color="#F5A623" />
                                </div>
                            </div>
                            <div slot="reference" class="boxText" style="height: 90px;">
                                <div v-for="(soles, index) in sole.pici" :key="index">
                                    ...
                                    <el-progress v-if="index === 0" :percentage="soles.proportion" :showText="false" :textInside="true" :strokeWidth="8" color="#1890FF" />
                                    <el-progress v-else-if="index === 1" :percentage="soles.proportion" :showText="false" :textInside="true" :strokeWidth="8" color="#5BD171" />
                                    <el-progress v-else-if="index === 2" :percentage="soles.proportion" :showText="false" :textInside="true" :strokeWidth="8" color="#F5A623" />
                                </div>
                            </div>
                        </el-popover>
                    </div>
            ...
            <el-table ref="wheatTable" class="newTable" maxHeight="276" border headerRowClassName="tableHead" :data="wheatList" style="margin-top: 10px;" :rowClassName="rowDelFlag" @row-dblclick="editwheat">
                ...
                <el-table-column label="批次" prop="whtBatch" width="110" />
        ...
        <mds-card :title="'豆粕'" :name="'soy'" :iconBg="'#5bd171'">
            ...
            <el-table ref="pulpTable" class="newTable" border maxHeight="275" headerRowClassName="tableHead" :data="soyList" :rowClassName="rowDelFlag" style="margin-top: 10px;" @row-dblclick="editsoy">
                <el-table-column label="日期" prop="pulpDate" width="110" :showOverflowTooltip="true" />
                <el-table-column label="物料" width="160" :showOverflowTooltip="true">
                    <template slot-scope="scope">
                        {{ scope.row.materialCode }}
                        {{ scope.row.materialName }}
                    </template>
                </el-table-column>
                <el-table-column label="领用粮仓" prop="foodHolderName" width="100" :showOverflowTooltip="true" />
                <el-table-column label="豆粕仓" prop="pulpHolderName" width="130" :showOverflowTooltip="true" />
                <el-table-column label="批次" prop="batch" :showOverflowTooltip="true" />
                <el-table-column label="起始" prop="startWeight" width="70" :showOverflowTooltip="true" />
                <el-table-column label="结束" prop="endWeight" width="70" :showOverflowTooltip="true" />
                <el-table-column label="数量" prop="useWeight" width="70" :showOverflowTooltip="true" />
                <el-table-column label="单位" prop="unit" width="50" :showOverflowTooltip="true" />
                <el-table-column label="入罐/出罐" prop="useType" width="90" :showOverflowTooltip="true" />
                <el-table-column label="操作人员" prop="changer" width="140" :showOverflowTooltip="true" />
                <el-table-column label="操作时间" prop="changed" width="160" :showOverflowTooltip="true" />
                <el-table-column label="操作" width="70" fixed="right">
                    <template slot-scope="scope">
        ...
        <audit-log />
        <el-dialog :closeOnClickModal="false" :title="MTitle" :visible.sync="dialogFormVisibleMai" width="450px" customClass="dialog__class">
            <div slot="title">
                {{ MTitle }}
            </div>
            <el-form ref="wheatstar" :model="wheat" size="small" :rules="wheatrulestar">
                <el-form-item label="麦粉批次：" :labelWidth="formLabelWidth" prop="whtBatch">
                ...
                <el-form-item label="起始数：" :labelWidth="formLabelWidth" prop="startWeight">
        ...
        <el-dialog :closeOnClickModal="false" :title="MTitle" :visible.sync="dialogFormVisibleMai2" width="450px" customClass="dialog__class">
            <div slot="title">
                {{ MTitle }}
            </div>
            <el-form ref="wheatend" :model="wheat" size="small" :rules="wheatrulend" @submit.native.prevent>
                ...
                <el-form-item label="结束数：" :labelWidth="formLabelWidth" prop="endWeight">
        ...
        <el-dialog :closeOnClickModal="false" :title="MTitle" :visible.sync="dialogFormVisibleMai3" width="450px" customClass="dialog__class">
            <div slot="title">
                {{ MTitle }}
            </div>
        ...
        <el-dialog :closeOnClickModal="false" :title="DRTitle" :visible.sync="dialogFormVisibleDouRu" width="450px" customClass="dialog__class">
            <div slot="title">
                {{ DRTitle }}
            </div>
            <el-form ref="rusoy" :model="rusoy" size="small" :rules="rusoyrules" @submit.native.prevent>
                <el-form-item label="领用粮仓：" :labelWidth="formLabelWidth" prop="foodHolderId">
        ...
        <el-dialog :closeOnClickModal="false" :title="DCTitle" :visible.sync="dialogFormVisibleDouChu" width="450px" customClass="dialog__class">
            <div slot="title">
                {{ DCTitle }}
            </div>
            <el-form ref="chusoy" :model="chusoy" size="small" :rules="chusoyrules" @submit.native.prevent>
                ...
                <el-form-item label="物料批次：" :labelWidth="formLabelWidth">
                    <el-input v-model="chusoy.soyMaterialstr" disabled />
                </el-form-item>
                <el-form-item label="起始数：" :labelWidth="formLabelWidth">
...
export default {
    ...
    methods: {
        drawPie(id, piciData, totalkg) {
            ...
            if (typeof piciData !== 'undefined') {
                ...
                piciData.map(item => {
                    ...
                    if (index % 3 === 0) {
                        this.$set(picisole, 'itemStyle', {
                            normal: { color: '#1890FF' },
                            emphasis: { color: '#6FAFEA' }
                        });
                    } else if ((index - 1) % 3 === 0) {
                        this.$set(picisole, 'itemStyle', {
                            normal: { color: '#5BD171' },
                            emphasis: { color: '#46ED64' }
                        });
                    } else if ((index - 2) % 3 === 0) {
                        this.$set(picisole, 'itemStyle', {
                            normal: { color: '#F5A623' },
                            emphasis: { color: '#FAB748' }
                        });
                    }
                    pici.push(picisole);
                    n = n + item.proportion;
        ...
        mainrules() {
            ...
            this.soyList.forEach(it => {
                ...
                if (Number(it.useWeight) <= 0) {
                    ty = false;
                    this.$warningTost('出罐数量不能为0');
                    return false;
                }
            });
            if (useType) {
                ty = false;
                this.$warningTost('豆粕没有出罐领用数据，请确认');
                return false;
            }
            return ty;
        },
        ...
        getMaiholdList(formHeader) {
            ...
            this.$http(
                ...
                if (res.data.code === 0) {
                    ...
                    this.$http(`${KJM_API.DOUMATERREALWHEATIME_API}`, 'POST', {
        ...
        getDouholdList(formHeader) {
            this.$http(
                ...
                if (res.data.code === 0) {
                    ...
                    // this
                    this.$http(`${KJM_API.DOUMATERREALTIME_API}`, 'POST', {
                        workShop: formHeader.workShop
                    }).then(({ data }) => {
                        if (data.code === 0) {
                            this.pulpListPici = data.listInfo;
        ...
        savestauts(resolve, reject) {
            this.$http(`${KJM_API.DOUMATERSTATUS_API}`, 'POST', {
                status: this.formHeader.submitStatus,
                orderHouseId: this.formHeader.orderHouseId,
        ...
        partialUpdates(formHeader, str) {
            let inState = '';
            this.$http(`${KJM_API.DOUMATERLIST_API}`, 'POST', {
        ...
        GetPuplList(formHeader) {
            this.$http(`${KJM_API.DOUMATERPULPLIST_API}`, 'POST', {
