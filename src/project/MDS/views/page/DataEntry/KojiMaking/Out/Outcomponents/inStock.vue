        ...
        <mds-card :title="'生产入库'" :name="'instock'">
            <el-table class="newTable" header-row-class-name="tableHead" :data="InStock" :row-class-name="RowDelFlag" border tooltip-effect="dark">
                ...
                <el-table-column width="125" label="入库酱醪量">
                    <template slot-scope="scope">
                        <p>
                            {{ (scope.row.sauceWeight = parseInt(((scope.row.pulpWeight * 1 + scope.row.wheatWeight * 1) * params.params1 + scope.row.saltWaterWeight * params.params2) / params.params3)) }}
                        </p>
                    </template>
                </el-table-column>
                <el-table-column width="150" label="入库批次">
                    ...
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.batch" :disabled="!(isRedact && scope.row.status !== 'submit' && scope.row.status !== 'checked')" size="small" placeholder="手工录入" maxlength="10" />
                    </template>
                </el-table-column>
    ...
    methods: {
        ...
        GetParams(factory) {
            this.$http(`${SYSTEMSETUP_API.PARAMETERLIST_API}`, 'POST', {
                factory: factory,
                type: 'ZQ_jianglao_canshu'
            }).then(({ data }) => {
                if (data.code === 0) {
                    data.dicList.forEach((item) => {
                        if (item.code === 'CANSHU1') {
                            this.params.params1 = Number(item.value);
                        }
                        if (item.code === 'CANSHU2') {
                            this.params.params2 = Number(item.value);
                        }
                        if (item.code === 'CANSHU3') {
                            this.params.params3 = Number(item.value);
                        }
                    });
                } else {
                    this.$errorToast(data.msg);
        ...
        InStockRul() {
            let ty = true;
            this.InStock.forEach(item => {
                if (item.sauceWeight && item.batch) {
                    //
                } else {
                    ty = false;
