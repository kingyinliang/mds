...
export default class Index extends Vue {
    ...
    changeOptions(flag: string) {
        if (flag === 'factory') {
            ...
            const item = this.potList.find(ele => ele.holderId === this.startForm.potNo);
    ...
    abc() {
        if (this.endForm.fullPot === '1') {
            const startData = this.dataList.find((item, index) => index === this.dataList.length - 1);
            this.endForm.fullPotAmount = this.potList.find(ele => ele.holderId === this.endForm.potNo).amount * 1000 + (Number(this.endForm.endAmount) - startData.startAmount);
        }
    }
    ...
    save2() {
        ...
        return new Promise(function(resolve, reject) {
            let result = { code: 0, msg: '' };
            Vue.prototype
                .$http(`${SQU_API.MATERIAL_IN_SAVE_API}`, `POST`, that.dataList)
                .then(res => {
                    result = { code: res.data.code, msg: res.data.code === 0 ? '保存成功' : res.data.msg };
                    if (res.data.code === 0) {
                        resolve(result);
    ...
    submit2() {
        ...
        return new Promise(function(resolve, reject) {
            Vue.prototype
                .$http(`${SQU_API.MATERIAL_IN_SUBMIT_API}`, `POST`, that.dataList)
                .then(res => {
                    result = { code: res.data.code, msg: res.data.code === 0 ? '提交成功' : res.data.msg };
                    if (res.data.code === 0) {
                        resolve(result);
