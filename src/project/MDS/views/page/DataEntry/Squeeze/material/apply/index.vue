...
export default class Index extends Vue {
    ...
    inPotEnd(deviceId: string, deviceName: string) {
        ...
        const startData = this.dataList.find(item => item.recordId === recordId);
        if (startData) {
            this.endForm = {
                deviceId,
                deviceName,
    ...
    changeOptions(flag: string) {
        if (flag === 'factory') {
            ...
            const item = this.fermentPotList.find(ele => ele.holderId === this.startForm.fermentPotNo);
            this.startForm.fermentPotName = item ? item.holderName : '';
            this.startForm.orderId = item ? item.orderId : '';
        } else if (flag === 'fermentPotModify') {
            const item = this.fermentPotList.find(ele => ele.holderId === this.modifyForm.fermentPotNo);
