<template>
    <el-form ref="timesForm" :inline="true" :model="readyDate" size="small" label-width="125px">
        <mds-card :title="'准备工时(单位：min)'" :name="'readyTimes'" class="header_main" style="padding: 0;">
            ...
            <div v-if="readyDate.classes === '' || readyDate.classes === '白班' || readyDate.classes === '多班'" class="marginL10px">
                <div class="box-card-title clearfix">
                    <h3><em class="point-icon" />白班</h3>
                </div>
                <el-row>
            ...
            <div v-if="readyDate.isCause == '1' && (readyDate.classes === '' || readyDate.classes === '中班' || readyDate.classes === '多班')" class="marginL10px">
                <div class="box-card-title clearfix">
                    <h3><em class="point-icon" />中班</h3>
                </div>
                <el-row>
                    <el-form-item label="工作开始时间：">
                        <el-date-picker
            ...
            <div v-if="readyDate.isCause == '1' && (readyDate.classes === '' || readyDate.classes === '夜班' || readyDate.classes === '多班')" class="marginL10px">
                <div class="box-card-title clearfix">
            ...
            <div class="marginL10px" style="padding-top: 20px;">
                <el-row>
                    <el-form-item label="交接班：">
                        <el-input v-model="readyDate.shift" style="width: 184px;" placeholder="手工录入" :disabled="!(isRedact && (readyDate.status === 'noPass' || readyDate.status === 'saved' || readyDate.status === ''))" />
                    </el-form-item>
                    <el-form-item label="班前会：" label-width="125px;">
                        <el-input v-model="readyDate.meeting" style="width: 184px;" placeholder="手工录入" :disabled="!(isRedact && (readyDate.status === 'noPass' || readyDate.status === 'saved' || readyDate.status === ''))" />
                    </el-form-item>
                    <el-form-item label="生产前准备：">
                        <el-input v-model="readyDate.prepared" style="width: 184px;" placeholder="手工录入" :disabled="!(isRedact && (readyDate.status === 'noPass' || readyDate.status === 'saved' || readyDate.status === ''))" />
                    </el-form-item>
                    <el-form-item label="生产后清场：">
                        <el-input v-model="readyDate.clear" style="width: 184px;" placeholder="手工录入" :disabled="!(isRedact && (readyDate.status === 'noPass' || readyDate.status === 'saved' || readyDate.status === ''))" />
                    </el-form-item>
                </el-row>
            </div>
        </mds-card>
                ...
                if (this.readyDate.classes === '白班') {
                    if (this.readyDate.dayStartLineDate && this.readyDate.dayStartDate && (this.readyDate.dayDinner || this.readyDate.dayDinner === 0) && this.readyDate.dayCauseDate && this.readyDate.dayEndDate) {
                        if ((toDate(this.readyDate.dayEndDate) - toDate(this.readyDate.dayCauseDate)) / 60000 < 0) {
                            ty = false;
                            this.$warningToast('准备时间白班工作结束时间不能早于停线时间');
                            return false;
                        }
                        if (
                            ...
                            ty = false;
                            this.$warningToast('准备时间白班工作开始时间<=开线时间<停线时间<=工作结束时间');
                            return false;
                        }
                    } else {
                        ty = false;
                        this.$warningToast('准备时间白班必填字段未填');
                        return false;
                    }
                    ...
                    if (this.readyDate.midCauseDate && this.readyDate.midStartDate && (this.readyDate.midDinner || this.readyDate.midDinner === 0) && this.readyDate.midCauseDate && this.readyDate.midEndDate) {
                        if ((toDate(this.readyDate.midEndDate) - toDate(this.readyDate.midCauseDate)) / 60000 < 0) {
                            ty = false;
                            this.$warningToast('准备时间中班工作结束时间不能早于停线时间');
                            return false;
                        }
                        if (
                            ...
                            ty = false;
                            this.$warningToast('准备时间中班工作开始时间<=开线时间<停线时间<=工作结束时间');
                            return false;
                        }
                    } else {
                        ty = false;
                        this.$warningToast('准备时间中班必填字段未填');
                        return false;
                    }
                    ...
                    if (this.readyDate.nightStartLineDate && this.readyDate.nightStartDate && (this.readyDate.nightDinner || this.readyDate.nightDinner === 0) && this.readyDate.nightCauseDate && this.readyDate.nightEndDate) {
                        if ((toDate(this.readyDate.nightEndDate) - toDate(this.readyDate.nightCauseDate)) / 60000 < 0) {
                            ty = false;
                            this.$warningToast('准备时间夜班工作结束时间不能早于停线时间');
                            return false;
                        }
                        if (
                            ...
                            ty = false;
                            this.$warningToast('准备时间夜班工作开始时间<=开线时间<停线时间<=工作结束时间');
                            return false;
                        }
                    } else {
                        ty = false;
                        this.$warningToast('准备时间夜班必填字段未填');
                        return false;
                    }
                    ...
                    if (
                        ...
                        ty = false;
                        this.$warningToast('准备时间白班和夜班必填字段未填');
                        return false;
                    }
                }
            }
