<template>
    <div class="header_main">
        <el-card class="searchCard  newCard ferCard">
            <el-form :inline="true" :model="formHeader" size="small" labelWidth="70px" class="topform sole_row">
        ...
        <el-card v-show="fastS" class="searchCard  newCard ferCard" style="margin-top: 5px; padding: 0 !important;">
                ...
                <el-col v-for="(item, index) in DataList" :key="index" :span="4">
                    <el-card class="dataList_item">
                        <h3 class="dataList_item_tit">
                            {{ item.holderNo }} -
                            <span style="color: rgb(51, 51, 51); font-weight: normal; font-size: 14px;">{{ item.holderStatus === '1' ? '入库中' : item.holderStatus === '0' ? '空罐' : item.holderStatus === '2' ? '满罐' : item.holderStatus === '3' ? '领用中' : '' }}</span>
                        ...
                        <div class="dataList_item_pot clearfix" style="position: relative;">
                            ...
                            <div class="dataList_item_pot_box">
                                <div class="dataList_item_pot_box1" style="display: flex; flex-wrap: wrap; align-content: flex-end; position: relative;">
                                    ...
                                    <div v-if="item.holderStatus === '1' || item.holderStatus === '2' || item.holderStatus === '3'" class="dataList_item_pot_detail">
                                        <p>{{ item.batch }}</p>
            ...
            <el-row>
                <el-pagination :currentPage="formHeader.currPage" :pageSizes="[18, 24, 30]" :pageSize="formHeader.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="formHeader.totalCount" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
            </el-row>
        </el-card>
        <el-dialog :closeOnClickModal="false" :visible.sync="GnDialogTableVisible" width="500px" customClass="dialog__class">
            <div slot="title">
                GN搅罐
            </div>
            <div>
                <el-form ref="Gnstar" size="small" :model="formGn" :rules="Gnrulestar" labelWidth="150px">
                    <el-form-item label="罐号：">
                        {{ formGn.holderName }}
                    </el-form-item>
                    <el-form-item label="搅罐开始时间：" prop="gnStartTime">
                        <el-date-picker v-model="formGn.gnStartTime" type="datetime" placeholder="请选择" style="width: 200px;" format="yyyy-MM-dd HH:mm" valueFormat="yyyy-MM-dd HH:mm" />
                    </el-form-item>
                    <el-form-item label="搅罐结束时间：" prop="gnEndTime">
                        <el-date-picker v-model="formGn.gnEndTime" type="datetime" placeholder="请选择" style="width: 200px;" format="yyyy-MM-dd HH:mm" valueFormat="yyyy-MM-dd HH:mm" />
                    </el-form-item>
        ...
        <el-dialog :closeOnClickModal="false" :visible.sync="JsbDialogTableVisible" width="500px" customClass="dialog__class">
            <div slot="title">
                JSB出库
            </div>
            <div>
                <el-form ref="Jsbstar" size="small" :model="formJsb" :rules="Jsbrulestar" labelWidth="150px">
                    ...
                    <el-form-item label="满灌时间：">
                        <el-date-picker v-model="formJsb.fullDate" type="datetime" placeholder="请选择" style="width: 200px;" format="yyyy-MM-dd HH:mm" valueFormat="yyyy-MM-dd HH:mm" />
                    </el-form-item>
                    <el-form-item label="备注：">
        ...
        <el-dialog :closeOnClickModal="false" :visible.sync="ZcDialogTableVisible" width="500px" customClass="dialog__class">
            <div slot="title">
                转储
            </div>
            <div>
                <el-form ref="Zcstar" size="small" :model="formZc" :rules="Zcrulestar" labelWidth="150px">
                    <el-form-item label="领用罐号：">
                        {{ formZc.holderName }}
                    </el-form-item>
                    <el-form-item label="物料：">
                    ...
                    <el-form-item label="满灌时间：">
                        <el-date-picker v-model="formZc.fullDate" type="datetime" placeholder="请选择" style="width: 200px;" format="yyyy-MM-dd HH:mm" valueFormat="yyyy-MM-dd HH:mm" />
                    </el-form-item>
                    <el-form-item label="备注：">
...
export default {
    ...
    methods: {
        ...
        godetails(item) {
            ...
            setTimeout(function() {
                that.$router.push({
                    name: `DataEntry-Sterilized-SemiFinishedProduct-detail`
                });
            }, 100);
        },
        // 获取工厂
