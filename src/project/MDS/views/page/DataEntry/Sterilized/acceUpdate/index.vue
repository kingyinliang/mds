        ...
        <mds-card title="辅料添加记录" name="addSupDateList">
            <el-table header-row-class-name="tableHead" class="newTable" :data="AddSupDate" border tooltip-effect="dark">
                ...
                <el-table-column label="调整后需求数量" width="120" prop="unit" :show-overflow-tooltip="true">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.adjustAmount" :disabled="!isRedact || scope.row.supStatus === '已确认' || scope.row.addStatus === '已添加' || scope.row.delFlag === '1'" placeholder="请输入" size="mini" @input="AddSupChange(scope.row)" />
                    </template>
                </el-table-column>
                <el-table-column label="单位" width="50" prop="unit" :show-overflow-tooltip="true" />
                <el-table-column label="批次" width="100" prop="batch" :show-overflow-tooltip="true" />
                <el-table-column label="领用数量" width="100" prop="receiveAmount" :show-overflow-tooltip="true" />
                <el-table-column label="备注" width="100">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.remark" :disabled="!isRedact || scope.row.supStatus === '已确认' || scope.row.addStatus === '已添加' || scope.row.delFlag === '1'" placeholder="请输入" size="mini" />
                    </template>
                </el-table-column>
            </el-table>
        </mds-card>
...
export default {
    ...
    methods: {
        ...
        Dblckick(row) {
            ...
            // }
            if (row.orderStatus !== 'submit' && row.orderStatus !== 'checked' && this.isRedact === true) {
                this.$refs.multipleTable.toggleRowSelection(row)
                this.ZengDateButton = true
            }
            // if (row.supStatus === '已确认') {
            //   this.btn = true
            // } else {
