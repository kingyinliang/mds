<template>
    <div class="header_main">
        <el-card class="searchCard" style="margin-bottom: 5px;">
            <el-row type="flex">
                <el-col>
                    <el-form :inline="true" size="small" :model="formHeader" labelWidth="70px" class="topform multi_row">
        ...
        <el-card class="searchCard newCard">
            ...
            <el-table ref="multipleTable" headerRowClassName="tableHead" :data="dataList" border tooltipEffect="dark" @selection-change="handleSelectionChange" @row-dblclick="Dblckick">
                ...
                <el-table-column label="辅料状态" width="80" prop="supStatus" :showOverflowTooltip="true" />
                <el-table-column label="订单号" width="120" prop="orderNo" :showOverflowTooltip="true" />
                <el-table-column label="杀菌锅" width="100" prop="panName" :showOverflowTooltip="true" />
                <el-table-column
                    label="物料"
                ...
                <el-table-column label="订单量" width="80" prop="planOutPut" :showOverflowTooltip="true" />
                <el-table-column label="单位" width="50" prop="unit" :showOverflowTooltip="true" />
                <el-table-column label="杀菌状态" width="120" prop="steStatus" :showOverflowTooltip="true">
                    ...
                    <template slot-scope="scope">
                        <el-select v-model="scope.row.steStatus" placeholder="请选择" size="mini" :disabled="!isRedact || scope.row.supStatus === '已确认'">
                            ...
                            <el-option label="异常订单" value="异常订单" />
                        </el-select>
                    </template>
                </el-table-column>
                <el-table-column label="状态备注" width="100" prop="steStatusRemake" :showOverflowTooltip="true">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.steStatusRemake" :disabled="!isRedact || scope.row.supStatus === '已确认'" placeholder="请输入" size="mini" />
                    </template>
                </el-table-column>
                <el-table-column label="SAP备注" width="80" prop="sapRemake" :showOverflowTooltip="true" />
                <el-table-column label="提交人员" width="80" prop="oprDate" :showOverflowTooltip="true" />
                <el-table-column label="提交时间" width="80" prop="oprter" :showOverflowTooltip="true" />
            </el-table>
            <el-row>
        ...
        <el-card class="newCard" style="margin: 5px 0;">
            ...
            <el-table headerRowClassName="tableHead" :data="AddSupDate" border tooltipEffect="dark">
                <el-table-column type="index" width="55" label="序号" :showOverflowTooltip="true" />
                <el-table-column label="添加状态" width="80" prop="addStatus" :showOverflowTooltip="true" />
                <el-table-column
                    label="物料"
                    :showOverflowTooltip="true"
                >
                ...
                <el-table-column label="需求数量" width="80" prop="planAmount" :showOverflowTooltip="true" />
                <el-table-column label="差额" width="80" prop="unit" :showOverflowTooltip="true">
                    <template slot-scope="scope">
                        {{ (scope.row.diffAmount = scope.row.adjustAmount * 1 - scope.row.planAmount * 1) }}
                    </template>
                </el-table-column>
                <el-table-column label="调整后需求数量" width="120" prop="unit" :showOverflowTooltip="true">
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.adjustAmount" :disabled="!isRedact || scope.row.supStatus === '已确认' || scope.row.addStatus === '已添加'" placeholder="请输入" size="mini" />
                    </template>
                </el-table-column>
                <el-table-column label="单位" width="50" prop="unit" :showOverflowTooltip="true" />
                <el-table-column label="批次" width="100" prop="batch" :showOverflowTooltip="true" />
                <el-table-column label="领用数量" width="100" prop="receiveAmount" :showOverflowTooltip="true" />
        ...
        <el-card class="newCard">
            ...
            <el-table headerRowClassName="tableHead" :rowClassName="RowDelFlag" :data="SupDate" border tooltipEffect="dark">
                ...
                <el-table-column label="添加数量" width="110" prop="addAmount" :showOverflowTooltip="true">
                    <template slot="header">
                        <i class="reqI">*</i><span>添加数量</span>
                    </template>
                    <template slot-scope="scope">
                        <el-input v-model="scope.row.addAmount" :disabled="!isRedact || scope.row.supStatus === '已确认' || scope.row.addStatus === '已添加'" placeholder="请输入" size="mini" />
                    </template>
                </el-table-column>
                <el-table-column label="单位" width="100" prop="unit" :showOverflowTooltip="true">
                    <template slot="header">
                    ...
                    <template slot-scope="scope">
                        <el-select v-model="scope.row.unit" placeholder="请选择" size="mini" :disabled="!isRedact || scope.row.supStatus === '已确认' || scope.row.addStatus === '已添加'">
                            <el-option label="请选择" value="" />
                            <el-option v-for="(item, index) in Unit" :key="index" :label="item.value" :value="item.code" />
                        </el-select>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="领用数量" width="80" prop="receiveAmount" :showOverflowTooltip="true" />
                ...
                <el-table-column label="操作" width="70" fixed="right">
                    <template slot-scope="scope">
                        <el-button class="delBtn" type="text" icon="el-icon-delete" size="mini" :disabled="!isRedact || scope.row.supStatus === '已确认' || scope.row.addStatus === '已添加'" @click="del(scope.row)">
                            删除
                        </el-button>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        GetMaterails(factory, id) {
            this.$http(`${STERILIZED_API.STE_SUP_MAT_LIST_API}`, 'POST', {
                factory: factory,
                orderNo: id
            }).then(({ data }) => {
                if (data.code === 0) {
        ...
        // 物料下拉
        GetMaterail(id) {
            this.$http(`${BASICDATA_API.SERCHSAPLIST_API}`, 'POST', {
                factory: id,
                param: '欣和半成品'
            }).then(({ data }) => {
