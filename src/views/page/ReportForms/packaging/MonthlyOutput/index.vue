...
<div class="header_main">
  <query-table
    ...
    :query-auth="'report:form:listProductM'"
    :column="column"
    :export-excel="true"
    :export-option="exportOption"
    @get-data-success="setTable"></query-table>
</div>
</template>
...
export default {
  ...
  methods: {
    setTable (data) {
      ...
      if (data.page.list.length > 0) {} else { return false }
      let productDate = this.$refs.queryTable.queryForm.productDate
      let month = productDate.substring(productDate.indexOf('-') + 1).split('')[0] === '0' ? productDate.substring(productDate.indexOf('-') + 1).slice(1) : productDate.substring(productDate.indexOf('-') + 1)
