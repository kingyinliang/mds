        ...
        <div class="box-item">
          <div class="box-item-top">
            <div class="box-item-title">
              ...
              <div class="box-item-title-state" :class="{diannopassBtn: item.beanStatus === 'noPass'}">状态：{{item.beanStatus === 'noPass'? '审核不通过':item.beanStatus === 'saved'? '已保存':item.beanStatus === 'submit' ? '已提交' : item.beanStatus === 'checked'? '通过':item.beanStatus === '已同步' ? '未录入' : item.beanStatus}}</div>
            </div>
            <div class="box-item-container">
              ...
              <div class="box-item-container-right">
                ...
                <div class="btn">
                  <el-button type="primary" size="mini" :class="{nopassBtn: item.beanStatus === 'noPass'}" @click="goPage('煮豆', item)">数据录入</el-button>
                </div>
              </div>
            </div>
          </div>
      ...
      if (id) {
        this.$http(`${BASICDATA_API.FINDORGBYID_API}`, 'POST', {deptId: id, deptName: '制曲'}, false, false, false).then(res => {
          if (res.data.code === 0) {
            ...
            if (!this.params.workShop && this.workshopList.length > 0) {
